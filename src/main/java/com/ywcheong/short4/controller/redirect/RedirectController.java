...
public class RedirectController {
    ...
    @GetMapping({"/{token}", "/{token}/{accessSecret}"})
    public ResponseEntity<SimpleMessageResponse> redirectURL(@PathVariable String token,
                                                             @PathVariable(required = false) String accessSecret) {
        if (accessSecret == null || accessSecret.isEmpty()) {
            accessSecret = null;
        }

        log.info("Redirect Controller :: /{token}/{accessSecret?} :: token [{}] accessSecret? [{}]", token, accessSecret != null);
